...
typedef RT_MODEL_quarc_visualization__T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * quarc_visualization_kuka_kr_5_sixx_r850_demo(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(quarc_visualization_kuka_kr__M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "tcpip://172.22.11.2:17001?nagle=no");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: quarc_visualization_kuka_kr_5_sixx_r850_demo -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  S = quarc_visualization_kuka_kr_5_sixx_r850_demo();
  if (rtmGetErrorStatus(S) != NULL) {
