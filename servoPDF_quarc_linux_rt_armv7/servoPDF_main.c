 ...
 *
 * Date:           Thu Oct 10 15:11:30 2024
 * Model version:  14.66
 * Matlab version: 23.2 (R2023b) 01-Aug-2023
 ****************************************************************************/
...
#ifndef SAVEFILE
# define MATFILE                       "servoPDF.mat"
#else
# define MATFILE                       STRINGIZE(SAVEFILE)
#endif

static const real_T RUN_FOREVER = -1.0;
typedef RT_MODEL_servoPDF_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * servoPDF(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(servoPDF_M, GBLbuf.message);
}
...
static void rt_OneStep(RT_MODEL *S)
{
  ...
  errmsg =                             /* Matfile logging */
    rt_UpdateTXYLogVars(servoPDF_M->rtwLogInfo, (servoPDF_M->Timing.t));
  if (errmsg != NULL) {
    strncpy(GBLbuf.message, errmsg, sizeof(GBLbuf.message));
    GBLbuf.message[sizeof(GBLbuf.message) - 1] = '\0';
    GBLbuf.stopExecutionFlag = 1;
    return;
  }
  ...
  main(int argc, char * argv[])
{
  ...
  double extra_arg1 = 2.0;
  double extra_arg2 = 0.2;
  ...
  for (count = 1; count < argc; ) {
    ...
    if ((strcmp(option, "-tf") == 0) && (count != argc)) {/* final time */
      ...
      argv[count-2] = NULL;
      argv[count-1] = NULL;
    } else if (strcmp(option, "-Kp") == 0 && count != argc) {
      int items = sscanf(argv[count++], "%lf%1s", &extra_arg1,
                         extraneous_characters);
      if (items != 1) {
        (void) fprintf(stderr, "Kp parameter must be a real scalar.\n");
        parseError = true;
        break;
      }
      ...
      argv[count-2] = NULL;
      argv[count-1] = NULL;
    } else if (strcmp(option, "-Kd") == 0 && count != argc) {
      int items = sscanf(argv[count++], "%lf%1s", &extra_arg2,
                         extraneous_characters);
      if (items != 1) {
        (void) fprintf(stderr, "Kd parameter must be a real scalar.\n");
        parseError = true;
        break;
      }
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: servoPDF -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  servoPDF_B.ModelArgument = (t_double) extra_arg1;
  servoPDF_B.ModelArgument1 = (t_double) extra_arg2;
  ...
  rt_StartDataLoggingWithStartTime(servoPDF_M->rtwLogInfo, 0.0, rtmGetTFinal
    (servoPDF_M), servoPDF_M->Timing.stepSize0, (&rtmGetErrorStatus(servoPDF_M)));
  if (rtmGetErrorStatus(S) == NULL) {
    fflush(stdout);
     ...
     * Cleanup and exit *
     ********************/
    rt_StopDataLogging(MATFILE, servoPDF_M->rtwLogInfo);
  } else {
    string_format(GBLbuf.message, sizeof(GBLbuf.message),
                  "Error starting data logging: %s", rtmGetErrorStatus(S));
    rtmSetErrorStatus(S, GBLbuf.message);
